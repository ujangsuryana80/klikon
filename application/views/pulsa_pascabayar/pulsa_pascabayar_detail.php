...
<?php
  ...
  if($status){
      $name='';
      $kode='';
    if($this->input->post('idData')==77){
      $name='HALO';
    }else if($this->input->post('idData')==78){
      $name='MATRIX';
      $kode= '';
    }else if($this->input->post('idData')==79){
      $name='THREE';
    }else if($this->input->post('idData')==80){
      $name='SMARTFREN';
    }else if($this->input->post('idData')==81){
      $name='XPLOR';
    }else if($this->input->post('idData')==76){
      $name='ESIA';
    }
...
?>
  ...
  <script type="text/javascript">
   $('#top-up').on('click', function(){     
      ...
      $.ajax({
          ...
          success: function(data) {
              $('#data-pulsa-pascabayar').html(data);
              $('#input002').val(imsg);
              $('#harga').val(iharga);
   ...
   $('#bayar').on('click', function(){
      ...
      var istatus2 = 'bayar';
    if(iidData){
        loadContentWithParams('pulsa_pascabayar.pulsa_pascabayar_detail2', { 
          inv : inv, 
          idData : iidData, 
          msg : imsg, 
          harga : iharga, 
