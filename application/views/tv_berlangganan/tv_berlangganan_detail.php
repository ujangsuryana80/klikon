...
<?php
  if($status){
      $name='';
      $kode='';
    if($this->input->post('idData')==35){
      $name='INDOVISION';
    }else if($this->input->post('idData')==36){
      $name='TOPTV';
      $kode= ".".$harga.";0";
    }else if($this->input->post('idData')==37){
      $name='OKEVISION';
    }else if($this->input->post('idData')==38){
      $name='TELKOMVISION';
    }
...
?>
  ...
  <script type="text/javascript">
   $('#top-up').on('click', function(){     
      ...
      $.ajax({
          type: 'POST',
          url: '<?php echo base_url()."tv_berlangganan/read_detail"; ?>',
          data: {idData : iidData,  status : istatus, msg : imsg, harga :iharga},
          timeout: 10000,
          success: function(data) {
              $('#datatvberlangganan').html(data);
              $('#input002').val(imsg);
              $('#harga').val(iharga);
   ...
   $('#bayar').on('click', function(){
      ...
      var istatus2 = 'bayar';
    if(iidData){
        loadContentWithParams('tv_berlangganan.tv_berlangganan_detail2', { 
          inv : inv, 
          idData : iidData, 
          msg : imsg, 
          harga : iharga, 
